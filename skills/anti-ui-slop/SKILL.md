...
## Keep the Product's Own Visual Language

UIZZE has no default visual theme. Derive hierarchy, density, navigation, color, and composition from the product task, platform, local system, and the structurally relevant parts of selected references. Reuse the local language when it is coherent; when it is weak, make the shortest functional change and leave unsupported aesthetic choices unresolved.

Do not answer uncertainty with a white canvas, dark dashboard, purple gradient, glass, bento grid, or card collection by default. Use real content and task behavior to create structure, keep visible copy short, and add boundaries or visual gestures only when they perform a product job. Never copy another product's exact layout, typography, illustrations, voice, or brand assets.
